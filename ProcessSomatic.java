	    			...
	    			try
	    			{
	    				...
	    				else
	    				{
	    					...
	    					if(isVCF)
	    					{
	    						...
	    						String[] formatContents = format.split(":");
	    						String[] normalContents = normal.split(":");
	    						String[] tumorContents = tumor.split(":");

	    						int fieldIndex = 0;
	    						for(String fieldName : formatContents)
	    						{
	    							if(fieldName.equals("DP4"))
	    							{
	    								// Parse out the normal read counts //
	    								if(normalContents.length > fieldIndex)
	    								{
	    									String[] normalDP4 = normalContents[fieldIndex].split(",");
	    									normalReads1 = Integer.parseInt(normalDP4[0]) + Integer.parseInt(normalDP4[1]);
	    									normalReads2 = Integer.parseInt(normalDP4[2]) + Integer.parseInt(normalDP4[3]);
	    								}

	    								// Parse out the tumor read counts //
	    								if(tumorContents.length > fieldIndex)
	    								{
	    									String[] tumorDP4 = tumorContents[fieldIndex].split(",");
	    									tumorReads1 = Integer.parseInt(tumorDP4[0]) + Integer.parseInt(tumorDP4[1]);
	    									tumorReads2 = Integer.parseInt(tumorDP4[2]) + Integer.parseInt(tumorDP4[3]);
	    								}


	    							}
	    							else if(fieldName.equals("RD"))
	    							{
	    								if(normalContents.length > fieldIndex)
	    									normalRD = Integer.parseInt(normalContents[fieldIndex]);
	    								if(tumorContents.length > fieldIndex)
	    									tumorRD = Integer.parseInt(tumorContents[fieldIndex]);
	    							}
	    							else if(fieldName.equals("AD"))
	    							{
	    								if(normalContents.length > fieldIndex)
	    									normalAD = Integer.parseInt(normalContents[fieldIndex]);
	    								if(tumorContents.length > fieldIndex)
	    									tumorAD = Integer.parseInt(tumorContents[fieldIndex]);
	    							}
	    							fieldIndex++;
	    						}

	    						// Rescue: Use RD and AD to infer readcounts if we don't have them //
	    						if(normalReads1 == 0 && normalReads2 == 0)
	    						{
	    							normalReads1 = normalRD;
	    							normalReads2 = normalAD;
	    						}
